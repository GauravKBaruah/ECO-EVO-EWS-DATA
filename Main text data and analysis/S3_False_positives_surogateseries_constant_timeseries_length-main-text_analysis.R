...
# false positive data set with the paper. It takes a long time due to the function that needs to create a parallel 500 time series per 1 replicate time series.
...
# NOTE = THIS SCRIPT CALLS 'surrogate_functions.R' and NOT 'surrogate_functions_variable_length.R'!!!!!!!
...
source('~/Dropbox/Zurich PhD Research/2_Chapter_2/J Animal Ecology codes/surrogate_functions.R')
...
for(i in 1:100){  tryCatch({
  Tau.traitg.AR.SD2[i]<-surrogate_ews_trait_ar1_sd(data = second.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
}, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.SD2[i]<-surrogate_ews_trait_sd(data = second.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR2[i]<-surrogate_ews_trait_ar1(data = second.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauSD_2[i]<-surrogate_ews_m( second.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauAR_2[i]<-surrogate_ews_m( second.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.AR.SD2[i]<-surrogate_ews_sd_ar1(data = second.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  #Tau.trait2[i]<-mean(EWS.gen.2[[i]]$Tau.Trait)
  #Tau.N2[i]<-mean(EWS.gen.2[[i]]$Tau.N)
  
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR.SD3[i]<-surrogate_ews_trait_ar1_sd(data = third.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR3[i]<-surrogate_ews_trait_ar1(data = third.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.SD3[i]<-surrogate_ews_trait_sd(data = third.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauSD_3[i]<-surrogate_ews_m( third.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauAR_3[i]<-surrogate_ews_m(third.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  
  for(i in 1:100){  tryCatch({
    Tau.AR.SD3[i]<-surrogate_ews_sd_ar1(data = third.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  ...
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR.SD4[i]<-surrogate_ews_trait_ar1_sd(data = fourth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR4[i]<-surrogate_ews_trait_ar1(data = fourth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.SD4[i]<-surrogate_ews_trait_sd(data = fourth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauSD_4[i]<-surrogate_ews_m(fourth.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauAR_4[i]<-surrogate_ews_m(fourth.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.AR.SD4[i]<-surrogate_ews_sd_ar1(data = fourth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  
  #Tau.trait4[i]<-mean(EWS.gen.2[[i]]$Tau.Trait)
  #Tau.N4[i]<-mean(EWS.gen.2[[i]]$Tau.N)
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR.SD5[i]<-surrogate_ews_trait_ar1_sd(data = fifth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR5[i]<-surrogate_ews_trait_ar1(data = fifth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.SD5[i]<-surrogate_ews_trait_sd(data = fifth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauSD_5[i]<-surrogate_ews_m(fifth.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauAR_5[i]<-surrogate_ews_m(fifth.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.AR.SD5[i]<-surrogate_ews_sd_ar1(data = fifth.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  ...
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR.SD6[i]<-surrogate_ews_trait_ar1_sd(data = six.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.AR6[i]<-surrogate_ews_trait_ar1(data = six.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.traitg.SD6[i]<-surrogate_ews_trait_sd(data = six.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauSD_6[i]<-surrogate_ews_m(six.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  TauAR_6[i]<-surrogate_ews_m(six.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  tryCatch({
  Tau.AR.SD6[i]<-surrogate_ews_sd_ar1(data = six.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  
  
  for(i in 1:100){  
    tryCatch({
  Tau.traitg.AR.SD7[i]<-surrogate_ews_trait_ar1_sd(data = sev.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
  }, error=function(e){})}
  for(i in 1:100){  
    tryCatch({
  Tau.traitg.AR7[i]<-surrogate_ews_trait_sd(data = sev.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  
    tryCatch({
  Tau.traitg.SD7[i]<-surrogate_ews_trait_ar1(data = sev.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  
    tryCatch({
  TauSD_7[i]<-surrogate_ews_m(sev.op[[i]]$N[500:530], winsize = 50, indicator= "sd",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){  
    tryCatch({
  TauAR_7[i]<-surrogate_ews_m(sev.op[[i]]$N[500:530], winsize = 50, indicator = "ar1",detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  for(i in 1:100){
    tryCatch({
  Tau.AR.SD7[i]<-surrogate_ews_sd_ar1(data = sev.op[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
...
# 
# a
var.0.06<-data.frame(Kendall.tau=c(TauAR_2,TauSD_2,Tau.AR.SD2,Tau.traitg.AR2,Tau.traitg.SD2,Tau.traitg.AR.SD2),
...
data.g.FP<-data.frame(rbind(var.0.06,var.0.1, var.0.2, var.0.3,var.0.4,var.0.5),
                             Genetic_variation =c(rep("0.05",each=nrow(var.0.06)), rep("0.1", each=nrow(var.0.1)),
                                                  rep("0.2",each=nrow(var.0.2)), rep("0.3",each=nrow(var.0.3)), 
                                                  rep("0.4",each=nrow(var.0.4)),rep( "0.5",each=nrow(var.0.5))) )

#save(data.g.FP,file="False_positives_constant_timeseries_genvar.RData")

best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

g1<-ggplot(data= data.g.FP, aes(y = Kendall.tau,x = variation, color= variation )) +geom_boxplot(alpha=0)+
...
for ( i in 1:100){
  tryCatch({
    Tau.traitR.AR.SD1[i]<-surrogate_ews_trait_ar1_sd(data = first.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){}) 
  print(i)}

for(i in 1: 100 ){  
  tryCatch({
    Tau.traitR.SD1[i]<-surrogate_ews_trait_sd(data = first.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    
  }, error=function(e){}) }

for(i in 1:100){ 
  tryCatch({
    Tau.traitR.AR1[i]<-surrogate_ews_trait_ar1(data = first.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
  }, error=function(e){}) 
}
for(i in 1:100){  
  tryCatch({
    TauSD_1[i]<-surrogate_ews_m( first.R0[[i]]$N[500:530], "sd",winsize = 50, detrending = "gaussian", boots =500)$false_p 
  }, error=function(e){}) }
for(i in 1:100){
  tryCatch({
    TauAR_1[i]<-surrogate_ews_m( first.R0[[i]]$N[500:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p 
  }, error=function(e){}) }
for ( i in 1:100){
  tryCatch({
    Tau.AR.SD1[i]<-surrogate_ews_sd_ar1(data = first.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})
  print(i)}
...
#dr2[i]<-mean(EWSR0.2[[i]]$KTauDR)
 ...
 for ( i in 1:100){
   tryCatch({
  Tau.traitR.AR.SD2[i]<-surrogate_ews_trait_ar1_sd(data = second.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
   }, error=function(e){}) 
   print(i)}
    
  for(i in 1: 100 ){  
    tryCatch({
  Tau.traitR.SD2[i]<-surrogate_ews_trait_sd(data = second.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  
  }, error=function(e){}) }
  
  for(i in 1:100){ 
    tryCatch({
  Tau.traitR.AR2[i]<-surrogate_ews_trait_ar1(data = second.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){}) 
  }
  for(i in 1:100){  
    tryCatch({
  TauSD_2[i]<-surrogate_ews_m( second.R0[[i]]$N[490:530], "sd",winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){}) }
  for(i in 1:100){
    tryCatch({
  TauAR_2[i]<-surrogate_ews_m( second.R0[[i]]$N[490:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){}) }
  for ( i in 1:100){
    tryCatch({
    Tau.AR.SD2[i]<-surrogate_ews_sd_ar1(data = second.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
    print(i)}
  ...
  #dr2[i]<-mean(EWSR0.2[[i]]$KTauDR)
    ...
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.AR.SD3[i]<-surrogate_ews_trait_ar1_sd(data = third.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
      }, error=function(e){})}
    
    for ( i in 1:100){  
      tryCatch({
  Tau.traitR.AR3[i]<-surrogate_ews_trait_ar1(data = third.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
      }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.SD3[i]<-surrogate_ews_trait_sd(data = third.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
      }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauSD_3[i]<-surrogate_ews_m(third.R0[[i]]$N[490:530],"sd", winsize = 50, detrending = "gaussian", boots =500)$false_p 
      }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauAR_3[i]<-surrogate_ews_m( third.R0[[i]]$N[490:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.AR.SD3[i]<-surrogate_ews_sd_ar1( third.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
  #dr3[i]<-mean(EWSR0.3[[i]]$KTauDR)
  
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.AR.SD4[i]<-surrogate_ews_trait_ar1_sd(data = fourth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.AR4[i]<-surrogate_ews_trait_ar1(data = fourth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.SD4[i]<-surrogate_ews_trait_sd(data = fourth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauSD_4[i]<-surrogate_ews_m(fourth.R0[[i]]$N[490:530],"sd", winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauAR_4[i]<-surrogate_ews_m( fourth.R0[[i]]$N[490:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.AR.SD4[i]<-surrogate_ews_sd_ar1( fourth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
  #dr3[i]<-mean(EWSR0.3[[i]]$KTauDR)
  
  #dr4[i]<-mean(EWSR0.4[[i]]$KTauDR)
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.AR.SD5[i]<-surrogate_ews_trait_ar1_sd(data = fifth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.AR5[i]<-surrogate_ews_trait_ar1(data = fifth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.traitR.SD5[i]<-surrogate_ews_trait_sd(data = fifth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauSD_5[i]<-surrogate_ews_m(fifth.R0[[i]]$N[490:530],"sd", winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  TauAR_5[i]<-surrogate_ews_m( fifth.R0[[i]]$N[490:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})}
    for ( i in 1:100){
      tryCatch({
  Tau.AR.SD5[i]<-surrogate_ews_sd_ar1( fifth.R0[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p }, error=function(e){})
      print(i)}
...
data.R.falsepositives<-data.frame(rbind(R1,R2, R3, R4,R5),
                             R0 =c(rep("1.1",each=nrow(R1)), rep("1.2", each=nrow(R2)),rep("1.3",each=nrow(R3)), rep("1.4",each=nrow(R4)), 
                                   rep("1.5",each=nrow(R5))) )

#save(data.R.falsepositives, file="False_positives_constant_timeseries_R0.RData")

best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

r1<-ggplot(data= data.R.falsepositives, aes(y = Kendall.tau,x = variation, color= variation )) +geom_boxplot(alpha=0)+
...
for(i in 1:100){
  tryCatch({ 
    Tau.traitP.AR.SD2[i]<-surrogate_ews_trait_ar1_sd(data = second.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
  }, error=function(e){})}
  
  for(i in 1:100){
    tryCatch({ 
      Tau.traitP.SD2[i]<-surrogate_ews_trait_sd(data = second.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
      
      for(i in 1:100){
        tryCatch({ 
          Tau.traitP.AR2[i]<-surrogate_ews_trait_ar1(data = second.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
        }, error=function(e){})}
          
          for(i in 1:100){
            tryCatch({ 
              TauSD_2[i]<-surrogate_ews_m(second.p[[i]]$N[500:530],"sd", winsize = 50, detrending = "gaussian", boots =500)$false_p
            }, error=function(e){})}
            for(i in 1:100){
              tryCatch({ 
                TauAR_2[i]<-surrogate_ews_m( second.p[[i]]$N[500:530],"ar1", winsize = 50, detrending = "gaussian", boots =500)$false_p
              }, error=function(e){})}
              for(i in 1:100){
                tryCatch({ Tau.AR.SD2[i]<-surrogate_ews_sd_ar1(data = second.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
                }, error=function(e){})}
  

  for(i in 1:100){
    tryCatch({ 
      Tau.traitP.AR.SD3[i]<-surrogate_ews_trait_ar1_sd(data = third.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
    for(i in 1:100){
      tryCatch({ Tau.traitP.AR3[i]<-surrogate_ews_trait_sd(data = third.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
      }, error=function(e){})}
      
      for(i in 1:100){
        tryCatch({ Tau.traitP.SD3[i]<-surrogate_ews_trait_ar1(data = third.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
        }, error=function(e){})}
                 
        for(i in 1:100){
          tryCatch({ 
        TauSD_3[i]<-surrogate_ews_m(third.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p 
          }, error=function(e){})}
        for(i in 1:100){
          tryCatch({ TauAR_3[i]<-surrogate_ews_m( third.p[[i]]$N[500:530], 'ar1',winsize = 50, detrending = "gaussian", boots =500)$false_p
          }, error=function(e){})}
          for(i in 1:100){
            tryCatch({ 
          Tau.AR.SD3[i]<-surrogate_ews_sd_ar1(data = third.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
            }, error=function(e){})}
  
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR.SD4[i]<-surrogate_ews_trait_ar1_sd(data = fourth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR4[i]<-surrogate_ews_trait_sd(data = fourth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.SD4[i]<-surrogate_ews_trait_ar1(data = fourth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
  for(i in 1:100){
    tryCatch({ 
  TauSD_4[i]<-surrogate_ews_m(fourth.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
  for(i in 1:100){
    tryCatch({ 
  TauAR_4[i]<-surrogate_ews_m(fourth.p[[i]]$N[500:530], 'ar1',winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})}
  for(i in 1:100){
    tryCatch({ 
  Tau.AR.SD4[i]<-surrogate_ews_sd_ar1(data = fourth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})}
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR.SD5[i]<-surrogate_ews_trait_ar1_sd(data = fifth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){}) 
   }
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR5[i]<-surrogate_ews_trait_sd(data = fifth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  
  for(i in 1:100){
    tryCatch({ 
      Tau.traitP.SD5[i]<-surrogate_ews_trait_ar1(data = fifth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
      }
    for(i in 1:100){
      tryCatch({ 
        TauSD_5[i]<-surrogate_ews_m(fifth.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p
      }, error=function(e){})
        }
        
      for(i in 1:100){
          tryCatch({ 
            TauAR_5[i]<-surrogate_ews_m(fifth.p[[i]]$N[500:530], 'ar1',winsize = 50, detrending = "gaussian", boots =500)$false_p
          }, error=function(e){})
            }
            
            for(i in 1:100){
              tryCatch({ 
                Tau.AR.SD5[i]<-surrogate_ews_sd_ar1(fifth.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
              }, error=function(e){})
              }
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR.SD6[i]<-surrogate_ews_trait_ar1_sd(data = six.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR6[i]<-surrogate_ews_trait_sd(data = six.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.SD6[i]<-surrogate_ews_trait_ar1(data = six.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauSD_6[i]<-surrogate_ews_m(six.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauAR_6[i]<-surrogate_ews_m(six.p[[i]]$N[500:530],'ar1', winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.AR.SD6[i]<-surrogate_ews_sd_ar1(data = six.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR.SD7[i]<-surrogate_ews_trait_ar1_sd(data = sev.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR7[i]<-surrogate_ews_trait_sd(data = sev.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.SD7[i]<-surrogate_ews_trait_ar1(data = sev.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauSD_7[i]<-surrogate_ews_m(sev.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauAR_7[i]<-surrogate_ews_m(sev.p[[i]]$N[500:530],'ar1', winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.AR.SD7[i]<-surrogate_ews_sd_ar1(data = sev.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR.SD1[i]<-surrogate_ews_trait_ar1_sd(data = e.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.SD1[i]<-surrogate_ews_trait_sd(data = e.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.traitP.AR1[i]<-surrogate_ews_trait_sd(data = e.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauSD_1[i]<-surrogate_ews_m(e.p[[i]]$N[500:530],'sd', winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  TauAR_1[i]<-surrogate_ews_m(e.p[[i]]$N[500:530],'ar1', winsize = 50, detrending = "gaussian", boots =500)$false_p 
    }, error=function(e){})
  }
  for(i in 1:100){
    tryCatch({ 
  Tau.AR.SD1[i]<-surrogate_ews_sd_ar1(data = e.p[[i]], winsize = 50, detrending = "gaussian", boots =500)$false_p
    }, error=function(e){})
    print(i) }
...
data.p.plasticity_fp<-data.frame(rbind(P2,P3,P4, P5,P6,P8),
                             Plasticity =c(rep("0.05",each=600), rep("0.1", each=600),rep("0.2",each=597), rep("0.3",each=600), 
                                           rep("0.4",each=600),rep( "0.8",each=600)) )

#save(data.p.plasticity_fp,file="False_positives_constant_timeseries.RData")


best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

p1<-ggplot(data= data.p.plasticity_fp, aes(y = Kendall.tau,x = variation, color= variation )) +geom_boxplot(alpha=0)+
