...
dynamics<-function(tmax,ind,omega,var.size,rho,var.U, var.Theta,r0,mean.breed,beta){
  ...
  for (t in 2:tmax) {
    ...
    mean.fitness[t] =  (sqrt(omega)/sqrt(var.size+omega))*exp(-(phenotype[t]-theta[t])^2/(2*(var.size+omega))) 
...
lownoise.trait.genvar.0.3<-lapply(1:reps, mc.replica,tmax=800,ind=100,omega=20,beta=0.2,var.size=0.3,rho=1,var.U=1.5^2, 
                                      var.Theta=1.5^2,r0=1.2,mean.breed=start.mean.breed)

save(lownoise.trait.genvar.0.3, file= "EWS.lownoise.genvar.0.3.RData")


#  s<-(EWS.trait.genvar.0.5[[1]]$phenotype) #-(EWS.trait.genvar.0.5[[1]]$phenotype)
# # 
#  par(mfrow=c(1,2))
#  plot(EWS.trait.genvar.0.5[[1]]$N[500:530], typ='l')
#  plot(s[500:530],typ='l')