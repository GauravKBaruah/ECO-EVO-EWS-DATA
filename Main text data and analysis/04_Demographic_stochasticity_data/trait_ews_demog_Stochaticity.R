...
load("EWS.demog.genvar0.05.RData") 
load("EWS.demog.genvar0.1.RData")  
load("EWS.demog.genvar0.2.RData") 
load("EWS.demog.genvar0.3.RData")
load("EWS.demog.genvar0.4.RData")
load("EWS.demog.genvar0.5.RData")
...
second.op<-EWS.demog.genvar.0.05
third.op<- EWS.demog.genvar.0.1 
fourth.op<- EWS.demog.genvar.0.2
fifth.op<-EWS.demog.genvar.0.3  
six.op<- EWS.demog.genvar.0.4
sev.op<-EWS.demog.genvar.0.5
...
data.g.demog<-data.frame(rbind(var.0.05,var.0.1, var.0.2, var.0.3,var.0.4,var.0.5),
...
save(data.g.demog,file="Demographic_stochasticity_genetic_variation.RData")
best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

g1.demog<-ggplot(data= data.g.demog, aes(y = Kendall.tau,x = Genetic_variation, color= Genetic_variation )) +geom_boxplot(alpha=0)+
...
#rm(list=ls())
load("EWS.demog.reproduction1.1.RData")
load("EWS.demog.reproduction1.2.RData") 
load("EWS.demog.reproduction1.3.RData")  
load("EWS.demog.reproduction1.4.RData")                       
load("EWS.demog.reproduction1.5.RData")

library("earlywarnings")



first.R0<-EWS.demog.reproduction.1.1
second.R0<-EWS.demog.reproduction.1.2#low genvariation low plasticity
third.R0<- EWS.demog.reproduction.1.3 #high genvariation low plasticity
fourth.R0<- EWS.demog.reproduction.1.4# low genvariation high plasticity
fifth.R0<-EWS.demog.reproduction.1.5
...
data.R.demog<-data.frame(rbind(R1,R2, R3, R4,R5),
                                  R0 =c(rep("1.1",each=nrow(R1)), rep("1.2", each=nrow(R2)),rep("1.3",each=nrow(R3)), rep("1.4",each=nrow(R4)), 
                                        rep("1.5",each=nrow(R5))) )

save(data.R.demog, file="Demographic_stochasticity_R0.RData")


best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

r1.d<-ggplot(data= data.R.demog, aes(y = Kendall.tau,x = R0, color= R0 )) +geom_boxplot(alpha=0)+
  geom_point(pch = 21,alpha=0.2, position = position_jitterdodge())+ylim(c(-1,1))+
  ylab("Kendall's Tau")+
  theme_classic()+theme(plot.title = element_text(size = 14,   face = "bold"),text = element_text(size = 12 ),
                        axis.title = element_text(face="bold"),axis.text.x=element_blank(),legend.position = "right") +
  scale_color_manual(values=best_color_paletter )+ labs(color="Reproductive rate")+
  facet_wrap(.~variation)+xlab("")
r1.d
...
load("EWS.demog.plasticity.0.05.RData") #low plasticity low genvariation
load("EWS.demog.plasticity.0.1.RData")  #low plasticity high genvariation
load("EWS.demog.plasticity.0.2.RData")      
load("EWS.demog.plasticity.0.3.RData")  #high plasticity low genvariation
load("EWS.demog.plasticity.0.4.RData")
load("EWS.demog.plasticity.0.5.RData")
load("EWS.demog.plasticity.0.8.RData")
...
second.p<-EWS.demog.plasticity.0.05#low genvariation low plasticity
third.p<- EWS.demog.plasticity.0.1 #high genvariation low plasticity
fourth.p<- EWS.demog.plasticity.0.2# low genvariation high plasticity
fifth.p<-EWS.demog.plasticity.0.3
six.p<- EWS.demog.plasticity.0.4
sev.p<-EWS.demog.plasticity.0.5#high genvariation high plasticity 
e.p<-EWS.demog.plasticity.0.8
...
# a
data.p.plasticity_demog<-data.frame(rbind(P2,P3,P4, P5,P6,P8),Plasticity =c(rep("0.05",each=600), rep("0.1", each=600),
                                               rep("0.2",each=600), rep("0.3",each=600), 
                                               rep("0.5",each=600),rep( "0.8",each=600)) )


save(data.p.plasticity_demog, file= "Demographic_stochasticity_plasticity.RData")

best_color_paletter<-c("#A42820", "#5F5647",  "#E58601",  "#00A08A" ,
                       "#4E2A1E","#000066", "#0C1707", "#000066")

p1<-ggplot(data= data.p.plasticity_demog, aes(y = Kendall.tau,x = Plasticity, color= Plasticity )) +geom_boxplot(alpha=0)+
  geom_point(pch = 21,alpha=0.2, position = position_jitterdodge())+ylim(c(-1,1))+
  ylab("Kendall's Tau")+
  theme_classic()+theme(plot.title = element_text(size = 14,   face = "bold"),text = element_text(size = 12 ),
                        axis.title = element_text(face="bold"),axis.text.x=element_blank(),legend.position = "right") +
